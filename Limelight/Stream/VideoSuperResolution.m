...
// Reports whether the MetalFX spatial scaler can actually run on this GPU. The scaler's own
// +supportsDevice: is the authoritative runtime check (it is not tied to a simple chip-family
// line), so we query it directly. Returns NO when MetalFX is missing from the SDK, the OS is
// older than 16.0, or no Metal device exists.
                                                   ...
                                                   CF_RETURNS_RETAINED
{
    ...
    // This method runs on the dedicated VSR serial queue (invoked from the synchronous decode
    // output handler), never on the main/UI thread, so waiting for GPU completion here adds no
    // input latency. It serializes GPU completion before the CVPixelBuffer is handed to
    // AVSampleBufferDisplayLayer and before the pool can recycle it, eliminating the
    // enqueue-before-blit-finished tear.
